...
describe('anitomy-js', function() {
  ...
  describe('anitomy parseSync', function() {
      ...
      expect(
          anitomy
              .parseSync(
                  ...
                  {"ignored_strings": ['!']})
              .file_name)
          .to.eq('[ANBU]_Princess_Lover_-_01_[2048A39A]');
    });
    ...
    it('should throw an exception for wrong datatypes (options)', function() {
      expect(function() {
        anitomy.parseSync("", null);
      }).to.throw('Options must be an object');
    });

    it('should throw an exception for wrong datatypes (options/ignored_strings)',
       function() {
         expect(function() {
  ...
  describe('anitomy parseAsync', function() {
       ...
       function(done) {
         ...
         anitomy.parseAsync(fixtureKeys, function(data) {
           data.forEach(function(anime) {
             expect(anime, anime.file_name)
                 .to.not.have.property("episode_title");
             // check episode_number_alt because anitomy still parsing
             // some file's episode number even with parse_episode_number set to
             // false
             expect(anime, anime.file_name)
                 .to.not.have.property("episode_number_alt");
             expect(anime, anime.file_name)
                 .to.not.have.property("file_extension");
             expect(anime, anime.file_name)
                 .to.not.have.property("release_group");
           });
           done();
         }, options);
       });
    ...
    it('should ignore string', function(done) {
      expect(
          anitomy.parseAsync(
              '[ANBU]_Princess_Lover!_-_01_[2048A39A].mkv', function(data) {
                expect(data.file_name)
                    .to.eq('[ANBU]_Princess_Lover_-_01_[2048A39A]');
                done();
              }, {"ignored_strings": ['!']}));
    });
          ...
          anitomy.parseAsync(
              ...
              function(data) {
                expect(data).to.deep.equal({
                  anime_title:
                      "Seikon no Qwaser - 13v0 (Uncensored Director's Cut)",
                  file_checksum: '988DB090',
                  file_extension: 'mkv',
                  file_name:
                      '[chibi-Doki] Seikon no Qwaser - 13v0 (Uncensored Director\'s Cut) [988DB090]',
    ...
    it('should throw an exception for wrong datatypes (files)', function() {
      expect(function() {
        anitomy.parseAsync(1, function() {});
      }).to.throw('Wrong data type');
    });

    it('should throw an exception for wrong datatypes (callback)', function() {
      expect(function() {
        anitomy.parseAsync("", null);
      }).to.throw('Second parameter must be a callback');
    });

    it('should throw an exception for wrong datatypes (options)', function() {
      expect(function() {
        anitomy.parseAsync("", function() {}, null);
      }).to.throw('Options must be an object');
    });
